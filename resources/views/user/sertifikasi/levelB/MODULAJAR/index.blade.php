...
<div class="max-w-2xl mx-auto mt-10 p-6 bg-white rounded-xl shadow-md dark:bg-gray-800">
    <h2 class="text-2xl font-bold text-gray-800 dark:text-white mb-6">Upload modul ajar & Deskripsi</h2>

    <form action="{{ route('asesi.sertifikasi.level.b.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('post')
        <div>
            <label for="modul_ajar" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Upload File modul ajar</label>
